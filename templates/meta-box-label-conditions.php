					...
					<div class="conditions-wrapper__row__inner" >
						<select name="conditions[<?php echo $key; ?>][type]" class="select-type">
							...
							foreach ( $default_conditions as $default_key => $default_value ) {
								?>
									<option value="<?php esc_attr( $default_key ); ?>" <?php selected( $default_key, $value['type'] ); ?>><?php esc_html_e( $default_value['label'] ); ?></option>
									<?php
							}
							?>
						</select>
						...
						foreach ( $default_conditions[ $value['type'] ]['compare'] as $default_key => $default_value ) {
							?>
								...
								<option value="<?php esc_attr( $default_key ); ?>" <?php selected( $default_key, $value['compare'] ); ?>><?php esc_html_e( $default_value ); ?></option>
								<?php
						}
						?>
						</select>
						...
						foreach ( $default_conditions[ $value['type'] ]['values'] as $default_key => $default_value ) {
							?>
								...
								<option value="<?php esc_attr( $default_key ); ?>" <?php selected( $default_key, $value['value'] ); ?>><?php esc_html_e( $default_value ); ?></option>
								<?php
						}
						?>
						</select>
